...
def read_data_full():
    f_train = open('dqn_data_10', 'r')
    f_labels = open('dqn_data_11', 'r')
#    f_ldb = open('/media/yokian/Storage/ML_COntest/DATASET/leaderboardTest_data', 'r')
#    print z.namelist()
...
def learn_mlp(X_train,Y_train,X_test, Y_test = None, learning_rate = 0.003
              ...
              , weights=None,biases = None,restore_save=False,num_layers = 1):
    #    print 'poop'
    #
    ## Parameters
    #learning_rate = 0.01
...
#weights = None
#biases = None       
Y_predicted0,e_list[0],a_list[0],weights,biases= learn_mlp(X_train,Y_train,X_test,Y_test,learning_rate = 0.003\
              , training_epochs = 3,n_hidden_1 = 2000,num_layers=1,restore_save=False, batch_size = 2^8,beta = 0*np.array([1,1,1,1]),weights=weights,biases=biases) # beta = [1e-7,1e-7,1e-7,1e-7]
